...
public class Lab9Application implements CommandLineRunner
{
	...
	private void optiune1()
	{
    	...
    	dao.insert(new Masina(nr_inmatriculare, marca, an_fabricatie, culoare, nr_km));
	...
	private void optiune2()
	{
    	...
    	Integer count = 0;
    	count = dao.deleteByNr_inmatriculare(nr_inmatriculare);
	...
	private void optiune3()
	{
    	...
    	find = dao.findByNr_inmatriculare(nr_inmatriculare);
	...
	private void optiune4()
	{
		...
		else
		{
			...
			for (var masina : dao.findAll()) logger.info("{}", masina);
		}
	...
	{
		Integer count = 0;
		count = dao.countByNr_km(-2, 100000);
	...
	private void optiune7()
	{
		...
		if (mod == 0)
		{
			...
			dao.findByAn_fabricatie(1, an).forEach(System.out::println);
		}
		else
		{
			logger.info("~~ lista cu masinile mai noi de 5 ani:");
			for (var masina : dao.findByAn_fabricatie(1, an)) logger.info("{}", masina);
		}
		System.out.println();
	}
