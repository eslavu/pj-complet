...
public class Lab9Application implements CommandLineRunner
{
	...
	private void optiune1()
	{
    	...
    	repository.save(new Masina(nr_inmatriculare, marca, an_fabricatie, culoare, nr_km));
	...
	{
		Integer count = 0;
		count = repository.getByNr_km(100000);
	...
	private void optiune7()
	{
		...
		if (mod == 0)
		{
			...
			repository.getByAn_fabricatie(an).forEach(System.out::println);
		}
		else
		{
			logger.info("~~ lista cu masinile mai noi de 5 ani:");
			for (var masina : repository.getByAn_fabricatie(an)) logger.info("{}", masina);
		}
		System.out.println();
	}
